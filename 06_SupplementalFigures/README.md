...
|-------|--------|-------------|
| S1 | `01_SupFig1.py` | Vaccine response by SMM 20/2/20 risk tier (LR / IR / HR), treatment-naive SMM only |
| S2 | `02_SupFig2.py` | Per-lineage cell-type annotation UMAPs and canonical marker-gene heatmaps |
| S3A | `03_SupFig3A.py` | Bone-marrow myeloid TNFSF13 (APRIL) across HD/MGUS/SMM/MM (Zavidij et al. 2020, GSE124310) |
| S3B | `03_SupFig3B.py` | Figure 3C reproduced on shipped samples only, holding shipment constant |
| S4A, S4B | `04_SupFig4AB.py` | APRIL-responsive signature in APRIL-stimulated plasmablasts, time-course heatmap and peak fold change (GSE173644) |
| S4C | `04_SupFig4C.py` | APRIL-responsive module in non-malignant bone-marrow plasma cells across HD/MGUS/SMM (Boiarsky et al. 2022, GSE193531) |
...
Where an external dataset carries no age or sex metadata (S3A, S4A-C), contrasts are unadjusted
two-sided Wilcoxon rank-sum tests and each script docstring says so.

## Shipment controls

S3B, S5 and S6 all read shipment status from `Shipment_FedEx` in Supplementary Table 4, where 1 is
...
Shipment varies only within SMM in the Figure 3C cohort: every healthy donor and all but one
participant with MGUS was shipped. S3B therefore reproduces Figure 3C using only samples
documented as shipped, holding shipment constant, and the disease comparison stays null.
...
| File | Panels |
|------|--------|
| `data/elisa/elisa_spike_post2nd.csv`, `elisa_spike_post3rd.csv` | S1 |
| `scRNAseq_IMPACT_Zenodo.h5ad` and the per-lineage objects in `SUBCLUSTER_DIR` | S2 |
| `data/metadata/Supplementary_Table_4_scRNAseq_sample_list.csv` and `scRNAseq_IMPACT_Zenodo.h5ad` | S3B, S6 |
| `data/external/zavidij_bm/matrices/` (GSE124310, downloaded from GEO) | S3A |
| `data/external/GSE173644_timecourse.txt.gz` | S4A, S4B |
| `data/external/GSE193531_umi-count-matrix.csv.gz` and `GSE193531_cell-level-metadata.csv` | S4C |
...
python3 04_SupFig4AB.py
python3 04_SupFig4C.py
