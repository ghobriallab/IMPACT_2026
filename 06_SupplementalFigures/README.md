...
| S3A | `03_SupFig3A.py` | Figure 3C reproduced on shipped samples only, the shipment control for Figure 3C |
| S3B | `03_SupFig3B.py` | Bone-marrow myeloid TNFSF13 (APRIL) across HD/MGUS/SMM/MM (Zavidij et al. 2020, GSE124310) |
...
Where an external dataset carries no age or sex metadata (S3B, S4A, S4B), contrasts are unadjusted
two-sided Wilcoxon rank-sum tests and each script docstring says so.

## Shipment controls

S3A, S5 and S6 all read shipment status from `Shipment_FedEx` in Supplementary Table 4, where 1 is
...
S3A reproduces Figure 3C using only samples documented as shipped, holding shipment constant.
...
participant was shipped. S3A and S6 each assert that they reproduce their parent panel before
reporting any stratified result. S5 reports an unadjusted two-sided Wilcoxon rank-sum test at each
timepoint, with clonotype proportions as percentages to match the Figure 4B axis.
...
| `data/metadata/Supplementary_Table_4_scRNAseq_sample_list.csv` and `scRNAseq_IMPACT_Zenodo.h5ad` | S3A, S6 |
| `data/external/zavidij_bm/matrices/` (GSE124310, downloaded from GEO) | S3B |
