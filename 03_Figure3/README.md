...
| 3D | `03_Figure3D.py` | SWIFT-seq (Lightbody et al., dbGaP phs003855): APRIL module in non-malignant plasma cells across NBM/MGUS/SMM/NDMM, bone marrow and peripheral blood. One sample per participant per compartment. The smaller Boiarsky GSE193531 version of this analysis is now Supplementary Figure 4B |
...
Panel 3C uses an age- and sex-adjusted rank-based ANCOVA versus HD, BH-corrected across the six contrasts of the panel. Panel 3D uses a two-sided Jonckheere-Terpstra ordered-trend test across NBM -> MGUS -> SMM -> NDMM, plus two-sided Wilcoxon rank-sum contrasts versus NBM with BH correction across the three contrasts of each compartment; the external dataset carries no age or sex metadata, so these contrasts are unadjusted. Panel 3E uses a two-sided paired Wilcoxon signed-rank test within each compartment, BH-corrected across the two compartments. Significance threshold q (or p) < 0.1.
