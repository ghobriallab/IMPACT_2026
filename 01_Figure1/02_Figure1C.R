# ============================================================================
# Purpose:      Figure 1C: SARS-CoV-2 spike IgG titers at the waning post-2nd-dose timepoint (2-4 months). Same age- and sex-adjusted ANCOVA + JT framework as Figure 1B.
# Inputs:       data/elisa/elisa_spike_post2nd.csv (de-identified ELISA titers).
# Outputs:      figures/Figure1C.png (and matching PDF).
...
# Filter: 2-4 months post-2nd dose
df <- df %>% filter(Days_post2nd > 60 & Days_post2nd <= 120)
...
p <- ggplot(plotdf, aes(x = Disease_Recoded, y = ELISA_Titer, fill = Disease_Recoded)) +
  ...
  ggtitle("2-4 months from 2nd dose") +
...
ggsave(file.path(FIGURES_DIR, "Figure1C.png"), bxp_padj, dpi = 300, unit = "in", width = 5.0, height = 4.0)
