...
# Full model: disease-specific intercepts AND slopes (random intercept per patient)
model_full <- lmer(
  log_titer ~ Disease + Days_post2nd:Disease + (1 | Common_ID),
  data = dat_serial,
  REML = FALSE
)

# Constrained model: disease-specific intercepts, common slope
model_constrained <- lmer(
  log_titer ~ Disease + Days_post2nd + (1 | Common_ID),
  data = dat_serial,
  REML = FALSE
)
